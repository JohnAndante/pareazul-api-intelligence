...
describe('MemoryService', () => {
    ...
    describe('setSessionCache', () => {
        it('should store session cache successfully', async () => {
            ...
            expect(mockRedis.setEx).toHaveBeenCalledWith(
                'chat_assistente_user123',
                3600,
                JSON.stringify(sessionData)
            );
            expect(mockLogger.debug).toHaveBeenCalledWith('Session cache stored for user user123');
        });
